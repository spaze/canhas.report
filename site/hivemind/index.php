...
header("Content-Security-Policy: default-src 'none'; script-src {$baseOrigin}; img-src {$baseOrigin}; style-src {$baseOrigin} 'nonce-{$nonce}'; base-uri 'none'; form-action 'none'");

$database = new PDO("mysql:host=$dbHost;dbname=$dbSchema", $dbUsername, $dbPassword);
$database->prepare('DELETE FROM reports WHERE received < ?')->execute([(new DateTime('-' . \Can\Has\dataRetentionDays() . ' days'))->format('Y-m-d H:i:s')]);
...
<!DOCTYPE html>
<html lang="en">
<?= \Can\Has\pageHead('All Received Reports', $nonce); ?>
<body>
